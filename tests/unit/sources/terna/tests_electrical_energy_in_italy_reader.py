"""Tests for the electrical_energy_in_italy table reader."""
...
TABLE = "electrical_energy_in_italy"
...
def test_terna_with_month_filter():
    """Reading with a month filter returns data for that month."""
    connector = _get_connector()

    records_iter, offset = connector.read_table(
        TABLE, None, {"year": "2023", "month": "1"}
    )
    records = list(records_iter)
