"""Tests for the electrical_energy_by_type table reader."""
...
TABLE = "electrical_energy_by_type"
...
def test_terna_invalid_type():
    """An invalid type value raises ValueError."""
    connector = _get_connector()

    with pytest.raises(ValueError, match="Invalid type value"):
        connector.read_table(TABLE, None, {"year": "2023", "type": "INVALID"})
...
def test_terna_with_type_filter():
    """Reading with a type filter returns only that type."""
    connector = _get_connector()

    records_iter, offset = connector.read_table(
        TABLE, None, {"year": "2023", "type": "Fonti rinnovabili"}
    )
    records = list(records_iter)
    ...
    for record in records:
        assert record.get("type") == "Fonti rinnovabili"
