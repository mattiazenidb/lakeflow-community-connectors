"""Tests for the electrical_energy_by_sector table reader."""
...
TABLE = "electrical_energy_by_sector"
...
def test_terna_invalid_sector():
    ...
    with pytest.raises(ValueError, match="Invalid sector value"):
        connector.read_table(TABLE, None, {"year": "2023", "sector": "INVALID"})
...
def test_terna_with_sector_filter():
    ...
    records_iter, offset = connector.read_table(
        TABLE, None, {"year": "2023", "sector": "Industria"}
    )
    records = list(records_iter)
    ...
    for record in records:
        assert record.get("sector") == "Industria"
