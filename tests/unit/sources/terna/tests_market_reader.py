"""Tests for the market_stat table reader."""
...
def test_terna_invalid_market():
    """An invalid market value raises ValueError."""
    connector = _get_connector()

    with pytest.raises(ValueError, match="Invalid market value"):
        connector.read_table(TABLE, None, {"year": "2023", "market": "INVALID"})
...
def test_terna_with_market_filter():
    """Reading with a market filter completes without error."""
    connector = _get_connector()

    records_iter, offset = connector.read_table(
        TABLE, None, {"year": "2023", "market": "Mercato libero"}
    )
    records = list(records_iter)
