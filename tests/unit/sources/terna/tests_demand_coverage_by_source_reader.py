"""Tests for the demand_coverage_by_source table reader."""
...
TABLE = "demand_coverage_by_source"
...
def test_terna_year_beyond_history_limit():
    ...
    with pytest.raises(ValueError, match="'year' must be within the last 5 solar years"):
        connector.read_table(TABLE, None, {"year": "2020"})


def test_terna_invalid_source():
    """An invalid source value raises ValueError."""
    connector = _get_connector()

    with pytest.raises(ValueError, match="Invalid source value"):
        connector.read_table(TABLE, None, {"year": "2024", "source": "INVALID"})
...
def test_terna_with_source_filter():
    """Reading with a source filter returns only that source."""
    connector = _get_connector()

    records_iter, offset = connector.read_table(
        TABLE, None, {"year": "2023", "source": "Fotovoltaico"}
    )
    records = list(records_iter)
    ...
    for record in records:
        assert record.get("source") == "Fotovoltaico"


def test_terna_with_region_filter():
    """Reading with a region filter returns only that region."""
    connector = _get_connector()
