...
def test_terna_full_missing_date_from():
    ...
    table_options = {"date_to": "29/10/2025", "types": "Photovoltaic"}
    start_offset = None

    with pytest.raises(ValueError, match="peak_valley_load_details requires 'date_from'"):
        records_iter, offset = connector.read_table("peak_valley_load_details", start_offset, table_options)
...
def test_terna_beyond_five_years_limit():
    ...
    connector = TernaLakeflowConnect(config)
    table_options = {"date_from": "31/12/2020", "types": "Photovoltaic"}
    start_offset = None
    ...
    with pytest.raises(ValueError, match="Terna connector: 'date_from' must be within the last 5 solar years, not sooner than 01/01/2021"):
        connector.read_table("peak_valley_load_details", start_offset, table_options)
...
def test_terna_cdc_more_than_sixty_days_multiple_chunks():
    ...
    date_from = (date_to - timedelta(days=2)).strftime("%d/%m/%Y")
    table_options = {"date_from": date_from, "types": "Photovoltaic"}
    start_offset = None
...
def test_terna_cdc():
    ...
    table_options = {"date_from": date_from, "types": "Wind"}
    start_offset = None
...
def test_terna_full_start_end_dates():
    ...
    connector = TernaLakeflowConnect(config)
    table_options = {"date_from": "01/02/2024", "date_to": "29/02/2024", "types": "Photovoltaic"}
...
def test_terna_not_full_start_end_dates():
    ...
    connector = TernaLakeflowConnect(config)
    table_options = {"date_from": "28/02/2024", "date_to": "29/02/2024", "types": "Photovoltaic"}
...
def test_terna_partial_start_end_dates():
    ...
    connector = TernaLakeflowConnect(config)
    table_options = {"date_from": "01/03/2024", "date_to": "17/03/2024", "types": "Photovoltaic"}
...
def test_terna_partial_start_end_dates_with_type():
    config_dir = Path(__file__).parent / "configs"
    config = load_config(config_dir / "dev_config.json")
...
def test_terna_partial_start_end_dates_with_multiple_types():
    config_dir = Path(__file__).parent / "configs"
    config = load_config(config_dir / "dev_config.json")
    ...
    connector = TernaLakeflowConnect(config)
    table_options = {"date_from": "01/03/2024", "date_to": "17/03/2024", "types": "Photovoltaic,Wind"}
    start_offset = {"cursor": "05/03/2024"}
